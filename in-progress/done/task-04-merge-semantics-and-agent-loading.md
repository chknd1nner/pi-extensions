...
- [x] **Step 1: Write the failing config-merge and agent-loader tests**
...
- [x] **Step 3: Implement recursive merge semantics**
...
- [x] **Step 4: Implement agent discovery, replacement-by-name, and validation**
...
## Prior review notes (superseded by final spec review)

✅ Approved. All 4 tests pass, typecheck clean. `deepMerge` correctly recurses objects and replaces arrays. `AgentLoader` correctly rejects `bash` and unknown tools at load time, applies user-local bundle replacement-by-name, handles ENOENT gracefully in both `readDirNames` and the user-dir fallback, and validates path traversal in capability roots. Implementation is faithful to the spec.
...
- The MVP security model only allows workspace-visible roots under `Inbox`, `Workspace`, and `Exports`; hidden/control-plane roots such as `.pi` and `.familyos` must remain unavailable.
- `services/familyos/src/config/agent-loader.ts` currently accepts hidden roots because it only rejects absolute paths and `..`, so an agent manifest can still declare `.pi` as a readable root.

Fresh verification evidence:
- A direct `npx tsx` spot check using an agent manifest with `readRoots: [".pi"]` returned `ALLOWED .../.pi/secret.txt` instead of rejecting the manifest or the root.

Fix direction for this task:
- tighten agent capability-root validation to reject hidden/control-plane roots in MVP
- add regression tests proving `.pi`, `.familyos`, and other non-workspace roots are rejected at load time

## Implementation notes

- `services/familyos/src/config/agent-loader.ts` now normalizes capability-root paths, still rejects absolute/upward-traversal paths, and rejects any manifest root whose top-level segment is outside the MVP workspace roots `Inbox`, `Workspace`, and `Exports`.
- `services/familyos/tests/agent-loader.test.ts` now includes regression coverage proving `.pi`, `.familyos`, and another non-workspace root (`Secrets`) are rejected at load time.
- Verification (RED): `cd services/familyos && npx vitest run tests/agent-loader.test.ts` failed first with 3 regressions, each showing `loader.loadAgent("invalid-roots")` resolved instead of rejecting for `.pi`, `.familyos`, and `Secrets` roots.
- Verification (GREEN): `cd services/familyos && npx vitest run tests/merge.test.ts tests/agent-loader.test.ts && npm run typecheck` passes (`7` tests passed, typecheck clean).

## Review approval

Approved. Re-review found no remaining blocking issues in Task 04 scope or in the documented spec divergence around MVP workspace roots.

Fresh verification evidence:
- `cd services/familyos && npx vitest run tests/merge.test.ts tests/agent-loader.test.ts && npm run typecheck` → passed (`7` tests passed, `tsc --noEmit` clean).

Scope check:
- `services/familyos/src/config/agent-loader.ts` now normalizes capability roots and rejects any top-level root outside `Inbox`, `Workspace`, or `Exports`, so control-plane roots like `.pi` and `.familyos` are blocked at load time.
- `services/familyos/tests/agent-loader.test.ts` adds regression coverage for `.pi`, `.familyos`, and another non-workspace root (`Secrets`), matching the spec’s MVP security boundary.
