...
- [x] **Step 1: Write the failing user-store tests**
...
- [x] **Step 3: Implement the user manifest lookup and home scaffolding**
...
- Verification run (RED): `cd services/familyos && npx vitest run tests/user-store.test.ts` failed as expected with missing `../src/identity/state-store` module.
- Verification run (GREEN): `cd services/familyos && npx vitest run tests/user-store.test.ts && npm run typecheck` passed.
- Follow-up concerns: none for this ticket scope.

## Review Notes
- The git diff matches the exact implementation specified in the ticket plan excerpt.
- The unit tests `tests/user-store.test.ts` pass cleanly (`vitest run`).
- The type check `npm run typecheck` passes cleanly.
- Good job sticking precisely to the scope. Approved!
