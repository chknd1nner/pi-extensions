...
next_prompt: |-
  Re-review Task 6 after fix.

  Fix applied:
  - `services/familyos/src/pi/familyos-extension.ts`: moved `consume()` to the top of the
    `before_provider_request` handler so the one-shot handoff is always cleared on the next
    outgoing request, regardless of whether the payload shape supports injection.
  - `services/familyos/tests/handoff.test.ts`: added regression test
    "consumes the handoff exactly once even when the payload has no system array" that
    asserts `handoff.peek()` is undefined and the audit event is recorded after the hook
    fires on an unsupported payload.

  Verification:
  - `cd services/familyos && npx vitest run tests/handoff.test.ts tests/runtime-factory.test.ts` ✅ (6 tests, was 5)
  - `cd services/familyos && npm run typecheck` ✅

  Problem to fix:
  - `services/familyos/src/pi/familyos-extension.ts` only clears the one-shot handoff after a successful payload injection.
  - When `before_provider_request` receives an unsupported payload shape (for example `{ system: "plain string" }`), it audits and leaves the handoff armed.
  - This violates the design spec requirement that the handoff fires on the next outgoing request only and is cleared after that request is built/sent.

  Required changes:
  1. Add a regression test covering the unsupported-payload path and asserting that the handoff is cleared after the hook runs, even when no injection occurs.
  2. Update the extension/handoff flow so unsupported payloads remain unchanged, still emit the audit event, and still consume the armed handoff exactly once.
  3. Re-run the targeted verification for this task and typecheck.

  Verification:
  - `cd services/familyos && npx vitest run tests/handoff.test.ts tests/runtime-factory.test.ts`
  - `cd services/familyos && npm run typecheck`
...
- [x] **Step 1: Write the failing handoff and runtime-factory tests**
...
- [x] **Step 3: Implement one-shot handoff storage and provider-payload rewriting**
...
- [x] **Step 4: Implement deterministic prompt composition and shared session-dir helpers**
...
- [x] **Step 5: Implement the FamilyOS extension factory**
...
- [x] **Step 6: Implement the runtime factory with selective same-name custom tools**
...
Approved. The handoff one-shot lifecycle now matches the spec.

Fresh verification evidence:

- `cd services/familyos && npx vitest run tests/handoff.test.ts tests/runtime-factory.test.ts` ✅ (6 tests passed)
- `cd services/familyos && npm run typecheck` ✅
- Manual repro with a `before_provider_request` payload shaped like `{ system: "plain string", messages: [] }` now leaves `handoff.peek()` empty and still records `handoff_payload_unsupported`.

Review notes:

- `services/familyos/src/pi/familyos-extension.ts` now consumes the handoff before attempting payload injection, so unsupported payload shapes still clear the one-shot state exactly once.
- `services/familyos/tests/handoff.test.ts` includes the regression coverage for the unsupported-payload path.

## Implementation notes

- Verification (RED): `cd services/familyos && npx vitest run tests/handoff.test.ts tests/runtime-factory.test.ts` failed first with missing modules (`../src/pi/handoff`) as expected.
- Verification (GREEN): `cd services/familyos && npx vitest run tests/handoff.test.ts tests/runtime-factory.test.ts && npm run typecheck` passes.
- Follow-up concern: `createInitialSessionManager()` currently falls back to `SessionManager.continueRecent(...)` when `activeSessionPath` is missing; in later tasks, ensure this fallback is surfaced in user-facing state handling and audit events where needed.
